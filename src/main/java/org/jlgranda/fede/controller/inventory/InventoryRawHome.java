...
@Named
@ViewScoped
public class InventoryRawHome extends FedeController implements Serializable {

    private static final long serialVersionUID = -21640696368253046L;

    Logger logger = LoggerFactory.getLogger(InventoryRawHome.class);
...
//        setStart(Dates.minimumDate(Dates.addDays(getLastProduct().getCreatedOn(),0)));
        ...
        getProduct().setProductType(ProductType.RAW_MATERIAL);
        setProductType(ProductType.RAW_MATERIAL);
        setMode("app.fede.chart.gap.total");

        setOutcome("inventory-raw-inbox");
        initializeActions();
    }
            ...
            if ("desactivar".equalsIgnoreCase(this.selectedAction)) {
                ...
                for (Product p : this.getSelectedProducts()) {
                    ...
                    try {
                        ...
                        java.util.logging.Logger.getLogger(InventoryRawHome.class.getName()).log(Level.SEVERE, null, ex);
                    }
                    break;
                }
                setOutcome("");
